...
foreach($result as $row)
{
    ...
    if($row['End_Date']<$date)
    {
      ...
      {
        $sub_array[] = '<a style="color:blue" href="../project/forumPM.php?Num_ID='.$row["Num_ID"].'">'.$name.'</a>';
        $sub_array[] = '<a style="color:blue">'.$row['Customer_id'].'</a>';
       //  $sub_array[] = '<a style="color:blue">'.$row['Customer_info'].'</a>';
        $sub_array[] = '<a style="color:blue" href="'.($row['SO_File']).'" >'.$row['SO_Num'].'</a>';
        $sub_array[] = '<a style="color:blue">'.$row['Start_Con'].'</a>';
      ...
      else if($row['Status'] == 'UAT' || $row['Status'] == 'complete' || $row['Status'] == 'hold' || $row['Status'] == 'Cancel' || $row['Status'] == 'Waiting Customer' ||  $row['Status'] == 'Waiting Revise SO' )
      {
        $sub_array[] = '<a  href="../project/forumPM.php?Num_ID='.$row["Num_ID"].'">'.$name.'</a>';
        $sub_array[] = '<a >'.$row['Customer_id'].'</a>';
       //  $sub_array[] = '<a style="color:blue">'.$row['Customer_info'].'</a>';
        $sub_array[] = '<a  href="'.($row['SO_File']).'" >'.$row['SO_Num'].'</a>';
        $sub_array[] = '<a>'.$row['Start_Con'].'</a>';
        $sub_array[] = '<a>'.$row['End_Date'].'</a>';
        $sub_array[] = '<a>'.$row['Service'].'</a>';
        $sub_array[] = '<a>'.$row['Sale_Name'].'</a>';
        $sub_array[] = '<a>'.$row['Pre_Sale'].'</a>';
        $sub_array[] = '<a>'.$row['PM_Name'].'</a>';
        // $sub_array[] = '<a href="'.($row['Confirm_UAT']).'" target="_blank"><img src="../project/Uploadfile/download-flat.png" width="40" height="40"></a>';
        // $sub_array[] = '<a href="'.($row['UAT']).'" target="_blank"><img src="../project/Uploadfile/download-flat.png" width="40" height="40"></a>';
        $sub_array[] = '<a>'.$row['Status'].'</a>';
        // $sub_array[] = '<a href="'.($row['Diagram']).'" target="_blank"><img src="../project/Uploadfile/download-flat.png" width="40" height="40"></a>';
        // $sub_array[] = '<button type="button" name="delete" id="'.$row["Num_ID"].'" class="btn btn-danger btn-xs delete">Delete</button>';
      ...
      {
        // $sql = "UPDATE pm_mnsp SET Status='Expire' WHERE Num_ID =$IDD";

        // $connect->query($sql);
      
        $sub_array[] = '<a style="color:red" href="../project/forumPM.php?Num_ID='.$row["Num_ID"].'">'.$name.'</a>';
        $sub_array[] = '<a style="color:red">'.$row['Customer_id'].'</a>';
       //  $sub_array[] = '<a style="color:red">'.$row['Customer_info'].'</a>';
        $sub_array[] = '<a style="color:red" href="'.($row['SO_File']).'" >'.$row['SO_Num'].'</a>';
        $sub_array[] = '<a style="color:red">'.$row['Start_Con'].'</a>';
